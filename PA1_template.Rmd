...
## **5.Comparing Activity Pattern Between Weekdays Weekends**
...
```
Before plotting we have to convert the interval values to a continuous form. The steps are similar to that done in (3).
```{r message=FALSE}
...
a <- ggplot(averageSteps, aes(time, log10(average))) + geom_line() +
              facet_grid(factor~.) +
              labs(x = "Interval", y = "Number of Steps", title = "Weekday and Weekend Activity Pattern")
print(a)
```
